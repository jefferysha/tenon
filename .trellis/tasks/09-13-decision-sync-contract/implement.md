...
4. 运行 `python3 .trellis/scripts/task.py validate --task-dir .trellis/tasks/09-13-decision-sync-contract`，检查 context 清单与 spec 路径；执行可用的 Markdown/文档 lint，并记录未实现的 `expired` 与 `humanGateSatisfied` 边界。
