...
- Filter chips are `role=radio` inside a `role=radiogroup` (roving tabindex, arrow keys); filter dropdowns are
  Radix menus with `menuitemradio` items; the gate switch is `role=radiogroup`.
