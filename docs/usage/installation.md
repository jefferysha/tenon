...
```bash
/usr/bin/curl -fsSL https://raw.githubusercontent.com/jefferysha/tenon/v0.1.7/install.sh | /bin/bash -s -- --codex --dry-run
```

The versioned script installs only prebuilt assets from the immutable stable
`v0.1.7` release. It never clones or compiles the source repository.
...
`v0.1.7/install.sh` command once per host. `tenon update` on 1.x reports a
