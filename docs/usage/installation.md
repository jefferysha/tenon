...
```bash
/usr/bin/curl -fsSL https://raw.githubusercontent.com/jefferysha/tenon/v1.1.3/install.sh | /bin/bash -s -- --codex --dry-run
```

The versioned script installs only prebuilt assets from the immutable stable
`v1.1.3` release. It never clones or compiles the source repository.
