/**
 * setup 命令 —— mock/真临时 fs 混合回归（full-install F3 骨架）。
...
describe('③runtime 分派 / 未知 sub', () => {
  test('已删除的 skills 子命令 → 未知 setup 子命令 + exit 1，零 exec 零 mutation', () => {
    const deps = makeDeps()
    const { env, calls } = spyEnv()
    expect(cmdSetup(deps, 'skills', { dryRun: true }, env)).toBe(1)
    expect(deps.errLines.join('\n')).toContain('未知 setup 子命令')
    expect(deps.errLines.join('\n')).toContain('支持: runtime')
    expect(calls.exec).toHaveLength(0)
    expect(calls.mkdirp).toHaveLength(0)
  })
...
describe('⑨空 sub 全流程 —— 宿主安装后接运行时就绪清单（dry-run 只提示不真探测,R1 concern#1）', () => {
  ...
  test('native update 计划在真实 managed runtime 发布后结束，不调用 setup runtime', async () => {
    const deps = makeDeps()
    const { env, calls } = spyEnv(
      { pathExists: setupPathExists, readText: setupReadText },
      codexInstallExec,
  ...
  test('非 dry-run:宿主安装之后真跑运行时就绪清单（注入 fakeRt,零真 docker）→ 出清单 + exit 0', async () => {
    const deps = makeDeps()
    // fake installer 只记录已校验候选发布，聚焦「运行时段确实被接上」。
    const { env } = spyEnv({ pathExists: setupPathExists, readText: setupReadText }, codexInstallExec)
    const runtime = fakeRuntimeInstaller()
    const rt = fakeRt({ hostEnv: { CLAUDE_CODE_OAUTH_TOKEN: 'a', OPENAI_API_KEY: 'b' } })
    const dashboard = fakeDashboardStarter()
    ...
    expect(code).toBe(0)
    const out = deps.outLines.join('\n')
    expect(out).toContain('就绪清单') // 运行时段跑了（一屏）
  ...
  test('--dry-run:运行时段只提示见 tenon setup runtime,绝不真探测 docker（同步返 number,不碰 rt）', () => {
    ...
    expect(out).toContain('--dry-run')
    expect(calls.exec).toHaveLength(0) // 零 exec（运行时段未探测）
    expect(authCalls).toBe(0)
  })
  ...
  test('宿主 marketplace 安装失败时立即退出，不会把未验证的插件伪装成可运行环境', async () => {
    const deps = makeDeps()
    let authCalls = 0
    // 全 exec 失败 → 宿主 marketplace 安装失败 → exit 1。
