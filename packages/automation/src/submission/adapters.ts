...
export function createDocumentProjectionAdapter(input: {
  ...
  return {
    record: async ({ subjectRef, path, documentKind, producer, recordedAt, allowBackfill, actor }) => {
      ...
      await recordDocument({ repoRoot: input.repoRoot, changeDir: input.changeDir, phase: input.phase, policy: input.policy, kind: documentKind as DocumentKind, path: repoRelativePath, producer, recordedAt, subjectRef, ...(allowBackfill !== undefined ? { allowBackfill } : {}), ...(actor !== undefined ? { actor } : {}) })
      return {}
    },
  }
}
