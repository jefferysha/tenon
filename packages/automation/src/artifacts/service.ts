...
export async function openArtifactService(options: ArtifactServiceOptions): Promise<ArtifactService> {
  ...
  async function versionFor(s: State, input: ObserveInput, bytes: Uint8Array, attempt: ArtifactAttempt): Promise<ArtifactVersion> {
    ...
    if (existing) {
      ...
      if (record !== undefined && input.origin === 'stage' && (existing.origin !== 'stage' || existing.producer === undefined)) {
        ...
        const adopted = { ...existing, origin: 'stage' as const, producer }
        s.artifacts = s.artifacts.map(r => r.artifactId === artifactId ? { ...r, versions: r.versions.map(v => v.version === existing.version ? adopted : v) } : r)
        await emit(s, 'artifact.observed', `observe:${input.idempotencyKey ?? `${attempt.stageAttemptId}:${artifactId}:${sha}`}`, { artifactId, version: existing.version, attemptId: attempt.stageAttemptId, payload: { source: input.observationSource ?? 'reconcile', ...(input.toolCallId ? { toolCallId: input.toolCallId } : {}) } })
        return adopted
      }
      await emit(s, 'artifact.observed', `observe:${input.idempotencyKey ?? `${attempt.stageAttemptId}:${artifactId}:${sha}`}`, { artifactId, version: existing.version, attemptId: attempt.stageAttemptId, payload: { source: input.observationSource ?? 'reconcile', ...(input.toolCallId ? { toolCallId: input.toolCallId } : {}) } })
      return existing
    ...
    await emit(s, 'artifact.observed', `observe:${input.idempotencyKey ?? `${attempt.stageAttemptId}:${artifactId}:${sha}`}`, { artifactId, version, attemptId: attempt.stageAttemptId, payload: { source: input.observationSource ?? 'reconcile', ...(input.toolCallId ? { toolCallId: input.toolCallId } : {}) } })
    return value
  }
  ...
  const service: ArtifactService = {
    ...
    read: async (id, artifactId, version, options = {}) => {
      ...
      const v = s.artifacts.find(x => x.artifactId === artifactId)?.versions.find(x => x.version === version); if (!v) throw new Error('artifact version not found')
