 ...
 */
export function Drawer({ open, onClose, title, actions, children, testId = 'drawer', ariaLabel }: DrawerProps): JSX.Element | null {
  const { t } = useT()
  ...
  return createPortal(
    <div className="fixed inset-0 z-50 flex justify-end" data-testid={`${testId}-root`}>
      ...
      <section
        ...
        tabIndex={-1}
        className={cn('flex h-full w-[560px] max-w-full flex-col border-l border-border bg-card shadow-lg outline-none', 'max-[900px]:w-full')}
        data-testid={testId}
      >
