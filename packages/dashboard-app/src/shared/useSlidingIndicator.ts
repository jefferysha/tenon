...
/** 默认当前项：单选芯片 / 分段（aria-checked）与导航当前页（aria-current=page）。 */
export const SLIDING_ACTIVE_SELECTOR = '[aria-checked="true"],[aria-current="page"]'
...
export interface SlidingIndicatorOptions {
  ...
  selector?: string
  /** 滑动时长（秒）；缺省 0.22（导航、芯片），分段控件用 0.18。 */
  duration?: number
}
...
export function useSlidingIndicator<C extends HTMLElement = HTMLDivElement>(
  ...
  useLayoutEffect(() => {
    ...
    mutations.observe(container, {
      ...
      characterData: true,
      attributes: true,
      attributeFilter: ['aria-checked', 'aria-current'],
    })
