...
export const FILTER_CHIP_CLS = 'inline-flex min-h-10 flex-none items-center gap-1 whitespace-nowrap rounded-sm px-2 text-body font-medium text-text-2 outline-none enabled:hover:bg-fill focus-visible:ring-2 focus-visible:ring-(--accent) aria-checked:bg-accent-t aria-checked:font-semibold aria-checked:text-(--accent)'
...
/** 单选芯片组：role=radiogroup + 可访问名；单行，不换行。 */
export function FilterChipGroup({
  ...
  return (
    <div className={cn('flex flex-none flex-nowrap items-center gap-1', className)} role="radiogroup" aria-label={label} data-testid={testId}>
      {children}
    </div>
  )
}
