...
describe('user dialog', () => {
  it('saving posts {id,name} to /api/user and reports the saved user', async () => {
    ...
    const onSaved = vi.fn()
    render(<I18nProvider><TooltipProvider><UserDialog onClose={() => undefined} onSaved={onSaved} /></TooltipProvider></I18nProvider>)
  ...
  it('allows saving an unedited git identity so it becomes the declared user.json', () => {
    window.__TENON_DASHBOARD_TOKEN__ = 'tok'
    render(<I18nProvider><TooltipProvider><UserDialog initial={{ id: 'jeff@x.io', name: 'Jeff', source: 'git' }} onClose={() => undefined} onSaved={() => undefined} /></TooltipProvider></I18nProvider>)
  ...
  it('stays disabled without a token and shows an error when the server rejects the id', async () => {
    render(<I18nProvider><TooltipProvider><UserDialog onClose={() => undefined} onSaved={() => undefined} /></TooltipProvider></I18nProvider>)
