...
function DropPreview({ label }: { label: string }): JSX.Element {
  return (
    <div data-testid="wb-skill-drop-preview" className="flex min-h-14 items-center justify-center rounded-lg border-2 border-dashed border-(--accent) bg-accent-t/55 px-4 text-base font-semibold text-accent-d shadow-[0_8px_28px_-22px_var(--accent)] transition-[background-color,border-color,color,box-shadow] duration-150 motion-reduce:transition-none">
      {label}
    </div>
  )
}
...
export function SkillOrchestrationDialog({
  ...
  return (
    <Dialog title={t('workbench.skill_dialog_title', { name: lane.name })} onClose={onClose} testid="wb-skill-orchestration" panelClassName="w-[min(1180px,96vw)]">
      <p className="mb-4 text-base leading-6 text-text-2">{t('workbench.skill_dialog_intro')}</p>
      <div className="grid min-h-[600px] grid-cols-[350px_minmax(0,1fr)] overflow-hidden rounded-lg border border-border bg-bg max-[820px]:grid-cols-1">
        <section className="border-r border-border bg-card p-4 max-[820px]:border-r-0 max-[820px]:border-b">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-title font-bold text-text">{t('workbench.skill_dialog_library')}</h2>
            <span className="ml-auto rounded-full bg-fill px-2 py-1 text-caption font-semibold text-text-3">{t('workbench.skill_dialog_available', { n: library.length })}</span>
          </div>
          <label className="mt-3 flex min-h-11 items-center gap-2 rounded-md border border-border bg-bg px-3 focus-within:border-(--accent) focus-within:ring-3 focus-within:ring-accent-t">
            <Search className="h-4 w-4 text-text-3" aria-hidden="true" />
            <span className="sr-only">{t('workbench.skill_dialog_search')}</span>
            <input name="skill-search" autoComplete="off" value={query} onChange={(event) => setQuery(event.target.value)} className="min-w-0 flex-1 bg-transparent text-base text-text outline-none" placeholder={t('workbench.skill_dialog_search_placeholder')} />
          </label>
          <div className="mt-3 max-h-[505px] space-y-2 overflow-y-auto pr-1">
            {registry === null || registry === undefined ? (
              <p className="rounded-md bg-fill p-4 text-base text-text-3" role="status" aria-live="polite">{t('workbench.skill_dialog_loading')}</p>
            ) : library.length === 0 ? (
              <p className="rounded-md bg-fill p-4 text-base text-text-3" role="status" aria-live="polite">{t('workbench.skill_dialog_empty')}</p>
            ) : library.map((entry) => {
              const presentation = skillPresentation(entry.name, registry, lang)
                ...
                >
                  <div className="flex items-start gap-3">
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <p className="mt-1 text-caption leading-5 text-text-3">{presentation.description}</p>
                      <div className="mt-2 flex items-center gap-2 text-micro text-text-3"><span>{t(entry.source === 'builtin' ? 'workbench.timeline_source_builtin' : entry.source === 'local-plugin' ? 'workbench.timeline_source_local' : entry.source === 'external-marketplace' ? 'workbench.timeline_source_marketplace' : 'workbench.timeline_source_user')}</span><span>·</span><span className={entry.installed ? 'text-green-d' : 'text-amber-d'}>{t(entry.installed ? 'workbench.timeline_installed' : 'workbench.timeline_uninstalled')}</span></div>
                    </div>
                    {canDrag && <span className="mt-1 inline-flex items-center gap-1 text-micro font-semibold text-accent-d opacity-0 transition-opacity group-hover:opacity-100 group-focus-visible:opacity-100">{t('workbench.skill_dialog_drag')} <ArrowRight className="h-3.5 w-3.5" aria-hidden="true" /></span>}
                  </div>
                </article>
              )
            })}
        ...
        >
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-title font-bold text-text">{t('workbench.skill_dialog_plan', { name: lane.name })}</h2>
            <span className="ml-auto text-caption text-text-3">{t('workbench.skill_dialog_plan_hint')}</span>
          </div>
          ...
          <ol className="relative mt-4 space-y-2">
            {skills.length === 0 && !dropTarget && <li className="rounded-md border border-dashed border-border-2 bg-card/60 p-10 text-center text-base text-text-3">{t('workbench.skill_dialog_plan_empty')}</li>}
              ...
              return (
                <li key={skillId} className="space-y-2">
                  ...
                  <article
                    ...
                    className="relative rounded-lg border border-border bg-card p-3 shadow-sm transition-[border-color,box-shadow,opacity,transform] duration-150 hover:border-accent-b hover:shadow-md data-[dragging]:scale-[.99] data-[dragging]:opacity-45 motion-reduce:transition-none"
                    title={presentation.technicalTitle}
                    onDragStart={(event) => {
                      event.stopPropagation()
                  ...
                  >
                    <div className="flex items-center gap-3">
                      ...
                      <span className="grid h-8 w-8 flex-none place-items-center rounded-full bg-(--accent) font-mono text-caption font-bold text-btn-fg shadow-sm">{index + 1}</span>
                      <div className="min-w-0 flex-1"><h3 className="font-semibold text-text">{presentation.name}</h3><p className="mt-0.5 text-caption text-text-3">{presentation.description}</p></div>
                      {!readonly && onMove && (
                        <span className="inline-flex gap-1">
                          <button type="button" className="grid h-9 w-9 place-items-center rounded-md text-text-3 hover:bg-fill hover:text-accent-d disabled:opacity-30" aria-label={t('workbench.move_skill_before', { name: presentation.name })} disabled={index === 0} onClick={() => {
                          ...
                          <button type="button" className="grid h-9 w-9 place-items-center rounded-md text-text-3 hover:bg-fill hover:text-accent-d disabled:opacity-30" aria-label={t('workbench.move_skill_after', { name: presentation.name })} disabled={index === skills.length - 1} onClick={() => {
                      ...
                      <div className="inline-flex rounded-md bg-fill p-1" role="group" aria-label={t('workbench.skill_dialog_mode', { id: skillId })}>
                        <button type="button" aria-label={t('workbench.skill_dialog_parallel_label', { id: skillId })} aria-pressed={mode === 'parallel'} disabled={readonly || !onDependencyChange} className="min-h-9 rounded-md px-3 text-caption font-semibold text-text-3 transition aria-pressed:bg-card aria-pressed:text-accent-d aria-pressed:shadow-sm" onClick={() => setExecutionMode(skillId, index, 'parallel')}>{t('workbench.skill_dialog_parallel')}</button>
                        <button type="button" aria-label={t('workbench.skill_dialog_serial_label', { id: skillId })} aria-pressed={mode === 'serial'} disabled={readonly || !onDependencyChange || index === 0} className="min-h-9 rounded-md px-3 text-caption font-semibold text-text-3 transition aria-pressed:bg-card aria-pressed:text-accent-d aria-pressed:shadow-sm disabled:opacity-35" onClick={() => setExecutionMode(skillId, index, 'serial')}>{t('workbench.skill_dialog_serial')}</button>
                        <button type="button" aria-label={t('workbench.skill_dialog_custom_label', { id: skillId })} aria-pressed={mode === 'custom'} disabled={readonly || !onDependencyChange} className="min-h-9 rounded-md px-3 text-caption font-semibold text-text-3 transition aria-pressed:bg-card aria-pressed:text-accent-d aria-pressed:shadow-sm" onClick={() => setExecutionMode(skillId, index, 'custom')}>{t('workbench.skill_dialog_dependencies')}</button>
                      </div>
                      <button type="button" className="grid h-9 w-9 place-items-center rounded-md text-text-3 hover:bg-red-t hover:text-red" aria-label={t('workbench.timeline_remove_skill', { id: presentation.name })} disabled={readonly || !onRemove} onClick={() => onRemove?.(lane.id, skillId)}><Trash2 className="h-4 w-4" aria-hidden="true" /></button>
                    </div>
                    {mode === 'custom' && (
                      <fieldset className="mt-3 border-t border-border pt-3">
                        <legend className="px-1 text-caption font-semibold text-text-2">{t('workbench.skill_dialog_dependency_legend')}</legend>
                        <div className="mt-2 flex flex-wrap gap-2">
                          {skills.filter((candidate) => candidate !== skillId).map((candidate) => {
                            const checked = deps.includes(candidate)
                            return <label key={candidate} className="inline-flex min-h-9 cursor-pointer items-center gap-2 rounded-md border border-border bg-bg px-3 text-caption text-text-2"><input name={`skill-dependency-${lane.id}-${skillId}-${candidate}`} type="checkbox" checked={checked} onChange={() => onDependencyChange?.(lane.id, skillId, checked ? null : candidate, checked ? candidate : null)} />{skillPresentation(candidate, registry, lang).name}</label>
                          })}
                        </div>
                      </fieldset>
                    )}
      ...
      <div className="mt-4 flex justify-end"><button type="button" className="min-h-10 rounded-md bg-(--accent) px-5 text-base font-semibold text-btn-fg hover:brightness-95" onClick={onClose}>{t('workbench.skill_dialog_done')}</button></div>
    </Dialog>
  )
}
