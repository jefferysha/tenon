...
export function OrchestrationV2Panel({ root, change, readOnly = false, onToast }: OrchestrationV2PanelProps): JSX.Element | null {
  ...
  return (
    <section className="prg-orchestration mt-4 rounded-md border border-border-2 bg-card/80 p-4 shadow-sm" data-testid="orchestration-v2-panel" aria-label={t('progress.orchestration_title')}>
        ...
        <div className="prg-orchestration-title">
          ...
          <div>
            <div className="flex items-center gap-2">
              <h2 className="text-base font-bold text-text">{t('progress.orchestration_title')}</h2>
              <span className={`text-caption font-semibold ${statusTone[status] ?? 'text-text-3'}`} data-testid="orchestration-v2-status">{status}</span>
              <span className="text-caption text-text-2">{statusLabel(status)}</span>
              <span className="text-micro text-text-3" data-testid="orchestration-v2-revision">rev {snapshot?.revision ?? '—'}</span>
            </div>
            <p className="mt-1 text-caption text-text-3">{connected ? t('progress.orchestration_connected') : t('progress.orchestration_syncing')} · {t('progress.orchestration_counts', counts)}</p>
          </div>
        </div>
      ...
      {error !== null && <p className="mt-2 text-caption text-red-700 dark:text-red-300" role="alert">{formatApiError(error, t, { exposeServerDetail: false })}</p>}
      ...
      {snapshot && snapshot.work_items.length > 0 && (
        <details className="prg-work-items mt-3" onToggle={(event) => setWorkItemsOpen(event.currentTarget.open)}>
          ...
          {snapshot.work_items.map((item) => {
            ...
            return <li key={item.work_item_id} className="prg-work-item rounded-md border border-border-2 px-3 py-2 text-caption">
              ...
              {!readOnly && ['failed', 'interrupted', 'blocked'].includes(item.status) && <div className="mt-2 flex flex-wrap gap-2"><button type="button" aria-label={`${t('progress.orchestration_retry')}: ${item.title}`} onClick={() => retryItem(item.work_item_id)} disabled={busy} className="rounded-xs border border-border-2 px-2 py-1 text-micro hover:bg-fill disabled:opacity-50">{t('progress.orchestration_retry')}</button><button type="button" aria-label={`${t('progress.orchestration_bind_artifact')}: ${item.title}`} onClick={() => bindArtifact(item.work_item_id)} disabled={busy} className="rounded-xs border border-border-2 px-2 py-1 text-micro hover:bg-fill disabled:opacity-50">{t('progress.orchestration_bind_artifact')}</button></div>}
              {artifactDraft?.workItemId === item.work_item_id && <form className="mt-2 grid gap-2 rounded-sm bg-fill/60 p-2" onSubmit={submitArtifact}><label className="grid gap-1 text-micro text-text-2" htmlFor={`artifact-ref-${item.work_item_id}`}>{t('progress.orchestration_artifact_ref')}<input id={`artifact-ref-${item.work_item_id}`} value={artifactDraft.ref} onChange={(event) => setArtifactDraft({ ...artifactDraft, ref: event.target.value })} className="rounded-xs border border-border-2 bg-card px-2 py-1 text-caption text-text" required /></label><label className="grid gap-1 text-micro text-text-2" htmlFor={`artifact-digest-${item.work_item_id}`}>{t('progress.orchestration_artifact_digest')}<input id={`artifact-digest-${item.work_item_id}`} value={artifactDraft.digest} onChange={(event) => setArtifactDraft({ ...artifactDraft, digest: event.target.value })} className="rounded-xs border border-border-2 bg-card px-2 py-1 text-caption text-text" placeholder="sha256:…" required /></label><div className="flex gap-2"><button type="submit" disabled={busy} className="rounded-xs border border-(--accent) px-2 py-1 text-micro hover:bg-fill disabled:opacity-50">{t('progress.orchestration_artifact_submit')}</button><button type="button" onClick={() => setArtifactDraft(null)} className="rounded-xs border border-border-2 px-2 py-1 text-micro hover:bg-fill">{t('progress.orchestration_artifact_cancel')}</button></div></form>}
            </li>
          })}
          </ul>}
        </details>
      )}
      {snapshot && <details className="prg-technical-details mt-3"><summary>{t('progress.orchestration_details')}</summary><div className="mt-2 grid gap-3 text-caption sm:grid-cols-2" data-testid="orchestration-v2-technical-details">
        <section className="rounded-md border border-border-2 p-3" aria-label={t('progress.orchestration_runs')}><h3 className="font-semibold text-text">{t('progress.orchestration_runs')}</h3>{snapshot.runs.length === 0 ? <p className="mt-1 text-text-3">—</p> : <ul className="mt-1 space-y-1">{snapshot.runs.map((run) => <li key={run.run_id} className="text-text-3">{run.skill_id}@{run.skill_version} · {run.status} · {run.attempt_id}{run.lease ? ` · ${run.lease.status}#${run.lease.generation}` : ''}</li>)}</ul>}</section>
