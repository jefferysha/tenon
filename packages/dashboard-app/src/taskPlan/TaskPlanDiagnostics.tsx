...
const boundedList = 'max-h-48 overflow-y-auto rounded-md border border-border bg-fill/35 p-2'
const code = 'break-all font-mono text-micro text-text-2 [overflow-wrap:anywhere]'
const empty = 'text-caption text-text-3'
...
function IdList({ ids, label, none }: { readonly ids: readonly string[]; readonly label: string; readonly none: string }): JSX.Element {
  ...
  return (
    <ul className="max-h-32 overflow-y-auto rounded-md border border-border bg-fill/35 p-2 flex flex-wrap gap-1.5" aria-label={label}>
      {ids.map((id) => <li key={id} className={`${code} rounded-sm bg-card px-2 py-1`}>{id}</li>)}
    </ul>
  )
}
...
function CoverageList({ entries, label }: { readonly entries: readonly TaskPlanCoverageEntry[]; readonly label: string }): JSX.Element {
  ...
  return (
    <ul className={`${boundedList} space-y-1.5`} aria-label={label}>
      {entries.map((entry) => (
        <li key={entry.id} className="min-w-0 rounded-sm bg-card px-2.5 py-2">
          <div className="flex min-w-0 flex-wrap items-baseline justify-between gap-2">
            <code className={`${code} font-semibold`}>{entry.id}</code>
            <span className="text-micro text-text-3">{t('task_plan.coverage_item_count', { count: entry.work_item_ids.length })}</span>
          </div>
...
export function TaskPlanDiagnostics({ plan }: TaskPlanDiagnosticsProps): JSX.Element {
  ...
  return (
    <section className="mt-5 min-w-0 border-t border-border pt-4" aria-labelledby="task-plan-diagnostics-title" data-testid="task-plan-diagnostics">
      <h3 id="task-plan-diagnostics-title" className="flex items-center gap-2 text-base font-semibold text-text">
      ...
      <div className="mt-4 grid min-w-0 gap-4 lg:grid-cols-2">
        <section className="min-w-0 rounded-md border border-border bg-fill/25 p-3" aria-labelledby="task-plan-coverage-title">
          <h4 id="task-plan-coverage-title" className="text-caption font-semibold text-text">{t('task_plan.coverage')}</h4>
          ...
          <div className="mt-3 grid min-w-0 gap-3">
            <div className="min-w-0">
              <h5 className="mb-1.5 text-micro font-semibold text-text-2">{t('task_plan.requirement_coverage')}</h5>
              <CoverageList entries={coverage.requirements} label={t('task_plan.requirement_coverage')} />
              <p className="mb-1.5 mt-3 text-micro font-semibold text-text-2">{t('task_plan.uncovered_requirements')}</p>
              <IdList ids={coverage.uncovered_requirement_ids} label={t('task_plan.uncovered_requirements')} none={t('task_plan.none')} />
            </div>
            <div className="min-w-0">
              <h5 className="mb-1.5 text-micro font-semibold text-text-2">{t('task_plan.acceptance_coverage')}</h5>
              <CoverageList entries={coverage.acceptance_criteria} label={t('task_plan.acceptance_coverage')} />
              <p className="mb-1.5 mt-3 text-micro font-semibold text-text-2">{t('task_plan.uncovered_acceptance')}</p>
              <IdList ids={coverage.uncovered_acceptance_ids} label={t('task_plan.uncovered_acceptance')} none={t('task_plan.none')} />
            </div>
          </div>
        </section>

        <section className="min-w-0 rounded-md border border-border bg-fill/25 p-3" aria-labelledby="task-plan-dependencies-title">
          <h4 id="task-plan-dependencies-title" className="flex items-center gap-2 text-caption font-semibold text-text">
            <GitBranch className="h-3.5 w-3.5 text-(--accent)" aria-hidden="true" />
            {t('task_plan.dependencies')}
          </h4>
          <p className="mb-1.5 mt-3 text-micro font-semibold text-text-2">{t('task_plan.dependency_edges')}</p>
          {dependencies.edges.length === 0 ? <p className={empty}>{t('task_plan.none')}</p> : (
            <ul className={`${boundedList} space-y-1.5`} aria-label={t('task_plan.dependency_edges')}>
              {dependencies.edges.map((edge) => (
                <li key={`${edge.from_work_item_id}:${edge.to_work_item_id}`} className={`${code} rounded-sm bg-card px-2.5 py-2`}>
                  {edge.from_work_item_id} → {edge.to_work_item_id}
                </li>
              ))}
            </ul>
          )}
          <p className="mb-1.5 mt-3 text-micro font-semibold text-text-2">{t('task_plan.cyclic_items')}</p>
          <IdList ids={dependencies.cyclic_work_item_ids} label={t('task_plan.cyclic_items')} none={t('task_plan.none')} />
        </section>

        <section className="min-w-0 rounded-md border border-border bg-fill/25 p-3" aria-labelledby="task-plan-resources-title">
          <h4 id="task-plan-resources-title" className="flex items-center gap-2 text-caption font-semibold text-text">
            <ShieldAlert className="h-3.5 w-3.5 text-(--accent)" aria-hidden="true" />
            {t('task_plan.resources')}
          </h4>
          <p className="mb-1.5 mt-3 text-micro font-semibold text-text-2">{t('task_plan.resource_conflicts')}</p>
          {resources.conflicts.length === 0 ? <p className={empty}>{t('task_plan.none')}</p> : (
            <ul className={`${boundedList} space-y-1.5`} aria-label={t('task_plan.resource_conflicts')}>
              {resources.conflicts.map((conflict) => (
                <li key={`${conflict.resource}:${conflict.work_item_ids.join(':')}`} className="min-w-0 rounded-sm bg-card px-2.5 py-2">
          ...
          <p className="mb-1.5 mt-3 text-micro font-semibold text-text-2">{t('task_plan.serialized_resources')}</p>
          {resources.serialized.length === 0 ? <p className={empty}>{t('task_plan.none')}</p> : (
            <ul className={`${boundedList} space-y-1.5`} aria-label={t('task_plan.serialized_resources')}>
              {resources.serialized.map((entry) => (
                <li key={`${entry.resource}:${entry.before_work_item_id}:${entry.after_work_item_id}`} className="min-w-0 rounded-sm bg-card px-2.5 py-2">
        ...
        <section className="min-w-0 rounded-md border border-border bg-fill/25 p-3" aria-labelledby="task-plan-validation-title">
          <h4 id="task-plan-validation-title" className="flex items-center gap-2 text-caption font-semibold text-text">
          ...
          {plan.validation.issues.length === 0 ? <p className={`${empty} mt-3`}>{t('task_plan.none')}</p> : (
            <ul className={`${boundedList} mt-3 space-y-2`} aria-label={t('task_plan.validation_issues')}>
              {plan.validation.issues.map((issue, index) => (
                <li key={`${issue.code}:${issue.path}:${index}`} className="min-w-0 rounded-sm bg-card px-2.5 py-2">
