...
export interface ArtifactCatalog { revision: number; digest: string; stageAttemptId: string; entries: ArtifactVersion[] }
...
function catalog(v: unknown): ArtifactCatalog | null {
  ...
  return { revision: v.revision, digest: v.digest, stageAttemptId: v.stageAttemptId, entries: entries as ArtifactVersion[] }
}
...
export async function fetchArtifactCatalog(root: string, stageAttemptId: string | undefined, options: { includeCandidates?: boolean; includeHistory?: boolean; maxEntries?: number; stageId?: string; change?: string } = {}): Promise<ArtifactCatalog> {
