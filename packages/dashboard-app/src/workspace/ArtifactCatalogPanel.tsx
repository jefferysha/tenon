...
function metadata(v: ArtifactVersion): string {
  const parts = [v.quality, `${v.size} B`, v.consumed ? 'consumed' : undefined, v.pendingUpdate ? 'pending update' : undefined, v.affected ? 'affected' : undefined, v.availableFromStage ? `from ${v.availableFromStage}` : undefined]
  return parts.filter((part): part is string => part !== undefined).join(' · ')
}

/** Runtime artifact view. It reads real catalog entries and never creates execution receipts. */
export function ArtifactCatalogPanel({ root, change, stageAttemptId, stageId, includeCandidates = false }: ArtifactCatalogPanelProps): JSX.Element {
  const { t } = useT()
