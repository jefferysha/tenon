...
export function ArtifactCatalogPanel({ root, change, stageAttemptId, stageId, includeCandidates = false, historyReference }: ArtifactCatalogPanelProps): JSX.Element {
  ...
  if (error) return <section className="rounded-md border border-border p-4 text-body text-red-d" role="alert">{error}</section>
  if (!catalog) return <section className="rounded-md border border-border p-4 text-body text-text-3" role="status">{t('workspace.runtime_artifacts_loading')}</section>
  ...
  return <section className="grid gap-3" data-testid="runtime-artifacts">
    ...
    {historyReference && <div className="rounded-md border border-border bg-card px-3 py-2 text-caption text-text-2" data-testid="runtime-artifacts-history-reference"><strong>{historyReference.lineageSource === 'legacy' ? t('workflow.runtime_artifacts_legacy_lineage') : t('workflow.runtime_artifacts_history_reference')}</strong><span className="ml-2">{historyReference.workflowRunId && historyReference.startedAt ? t('workflow.runtime_artifacts_history_detail', { attempt: historyReference.stageAttemptId, run: historyReference.workflowRunId, date: historyReference.startedAt }) : t('workflow.runtime_artifacts_provenance_unavailable')}</span></div>}
    {subjects && subjects.records.length > 0 && <div className="rounded-md border border-border bg-card px-3 py-2 text-caption text-text-3">subject projections · {subjects.records.length} · {subjects.records.map((record) => `${record.projection}:${record.status}`).join(' · ')}</div>}
    {catalog.entries.length === 0 ? <p className="rounded-md border border-dashed border-border px-3 py-4 text-body text-text-3">{t('workspace.runtime_artifacts_empty')}</p> : <ul className="grid gap-3">{Object.entries(grouped).map(([subjectId, entries]) => <li key={subjectId} className="grid gap-2"><div className="font-mono text-micro text-text-3">subject · {subjectId}</div><ul className="grid gap-2">{entries.map((v) => <li key={`${v.artifactId}:${v.version}`}><button type="button" className="grid w-full grid-cols-[minmax(0,1fr)_auto] gap-2 rounded-md border border-border bg-card px-3 py-2 text-left hover:border-accent-b" onClick={() => void open(v)}><span className="min-w-0"><span className="block truncate font-mono text-body text-text">{label(v)}<span className="ml-2 text-caption text-text-3">{v.version}</span></span><span className="block truncate text-micro text-text-3">{metadata(v)}</span></span><span className={`text-caption ${tone(v)}`}>{v.disposition}{v.pendingUpdate ? ' · pending update' : ''}{v.affected ? ` · ${t('workspace.runtime_artifacts_affected')}` : ''}</span></button></li>)}</ul></li>)}</ul>}
    {selected && <div className="rounded-md border border-border bg-card p-3" data-testid="artifact-preview"><div className="mb-2 flex justify-between text-caption text-text-2"><span>{label(selected)} · {selected.version}</span><button type="button" className="underline" onClick={() => setSelected(null)}>{t('detail.close')}</button></div>{content === null ? <p className="text-body text-text-3">{t('workspace.runtime_artifacts_reading')}</p> : <pre className="max-h-72 overflow-auto whitespace-pre-wrap text-caption text-text">{content}</pre>}</div>}
  </section>
}
