...
export interface ArtifactCatalogPanelProps {
  ...
  historyReference?: { readonly workflowRunId?: string; readonly stageAttemptId: string; readonly observedAt?: string }
}
...
export function ArtifactCatalogPanel({ root, change, stageAttemptId, stageId, includeCandidates = false, historyReference }: ArtifactCatalogPanelProps): JSX.Element {
  ...
  return <section className="grid gap-3" data-testid="runtime-artifacts">
    ...
    {historyReference && <div className="rounded-md border border-border bg-card px-3 py-2 text-xs text-text-2" data-testid="runtime-artifacts-history-reference"><strong>{t('workflow.runtime_artifacts_history_reference')}</strong><span className="ml-2">{t('workflow.runtime_artifacts_history_detail', { attempt: historyReference.stageAttemptId, run: historyReference.workflowRunId ?? historyReference.stageAttemptId, date: historyReference.observedAt ?? catalog.entries[0]?.createdAt ?? '—' })}</span></div>}
