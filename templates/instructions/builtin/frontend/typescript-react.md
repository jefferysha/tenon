...
TypeScript 7 + React 19 + Vite 8 + pnpm 12 + Axios，开发与构建环境为 Node.js 24 LTS。
...
- 全局状态与样式按本章「状态管理」「样式」小节执行；未选择时组件内状态用 `useState`，样式沿用项目现有方案。
...
- 遵守 Hooks 规则（启用 `eslint-plugin-react-hooks`）；`useEffect` 只用于与外部系统同步，派生数据直接计算，
  不要用 effect 同步 state。
- 状态就近放置：组件内状态用 `useState`；跨组件共享才进全局状态，按 feature 拆分，组件只订阅所需字段。
...
- 每个异步界面显式处理加载、空数据、错误、成功四种状态；应用壳与每个路由挂 Error Boundary。
- 页面组件按路由用 `React.lazy` + `Suspense` 拆包；`ref` 直接作为 prop 传递，新代码不写 `forwardRef`。
- 禁止 `dangerouslySetInnerHTML` 渲染未净化的内容；确需富文本时先经 DOMPurify 净化。
- 可访问性：交互元素使用语义标签或组件库组件，图片有 `alt`，表单控件有 label，可键盘操作。
- 使用 ESLint 10（`eslint.config.js`）+ Prettier，提交前 `pnpm lint`、`pnpm typecheck`、`pnpm test` 必须通过。
...
| 其他 `.ts`（hooks、store、api、utils） | {{module.soft}} 行 | {{module.hard}} 行 |
...
- 单元与组件测试用 Vitest 5 + React Testing Library（`@testing-library/react`、`@testing-library/user-event`）+ jsdom；
  测试文件与被测文件同目录，命名 `*.test.ts` / `*.test.tsx`。
- 查询元素优先 `getByRole` / `getByLabelText`，禁止按 CSS 类名或 DOM 层级断言。
- 每个 hook、store、`utils` 纯函数、`shared/api` 的解包与错误分支必须有单元测试；组件测试覆盖加载、空数据、错误、成功四种状态。
